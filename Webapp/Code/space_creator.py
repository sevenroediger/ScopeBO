...
def create_search_space_web(reactants,
                        reactants_names):

    """
    Webapp version of create_search_space()
    takes in list of dfs instead of files as reactants variable
    does not suggest samples or save data
    """
    
    list_of_dfs = []
    for df_component, component_name in zip(reactants, reactants_names):

        # prepare data
        df_component = df_component.dropna(axis='columns', how='all')

        # Set data types for all columns.
        df_component.iloc[:,:] = df_component.iloc[:,:].astype(float)

        # Add labels for the column names according to the reaction component.
        column_names = df_component.columns.tolist()
        column_names = [component_name+"_"+str(name) for name in column_names]
        df_component.columns = column_names
        # Add DataFrame to list.
        list_of_dfs.append(df_component)
    ...
    # Get the reactant combinations for each row.
    names_comb = list(it_product(*(df.index.tolist() for df in list_of_dfs)))
    names_comb = [".".join(name) for name in names_comb]
