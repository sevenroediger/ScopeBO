...
def get_covar_matrix(train_x):

    """
    Section of the build_and_optimize_model function above to just get the covariance matrix of the prior (needed for the Vendi score).
    ------------------------------------------------------------------------------------------------------
    train_x: tensor
        tensor of data points for which the covariance matrix is calculated.
    """
    ...
    try:
        ls_init = gp_options['ls_prior3']
        covar_module.base_kernel.lengthscale = ls_init
    except:
        uniform = gp_options['ls_prior3']
        ls_init = torch.ones(n_features).to(**tkwargs) * uniform
        covar_module.base_kernel.lengthscale = ls_init
